...
describe('Runtime check contract sync', () => {
    test('runtime contract defines all checks with required policy fields', () => {
        ...
        expect(definitionCheckIds.length).toBe(51);
        expect(contractCheckIds.length).toBe(51);
    ...
    test('heading ownership split and rewrite target policies match product contract', () => {
        ...
        expect(contractChecks.orphan_headings?.analysis_engine).toBe('ai');
        expect(contractChecks.heading_fragmentation?.analysis_engine).toBe('deterministic');
        expect(contractChecks.orphan_headings?.allowed_scopes).toEqual(['span']);
        expect(contractChecks.orphan_headings?.rewrite_target_policy).toBe('heading_support_range');
