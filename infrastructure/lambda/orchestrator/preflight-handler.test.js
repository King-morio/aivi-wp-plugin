...
describe('Preflight Handler', () => {
  ...
  describe('performDeterministicChecks', () => {
    ...
    test('should pass single H1 check when visible title exists outside body content', async () => {
      const manifest = {
        metadata: { h1_count: 0 },
        jsonld: [],
    ...
    test('should fail no H1 check', async () => {
      ...
      expect(checks.single_h1.explanation).toContain('No H1 tag or visible article title');
    });
    ...
    test('should keep missing schema and freshness/link scope checks neutral instead of awarding free deterministic credit', async () => {
      const manifest = {
        metadata: { h1_count: 1 },
        jsonld: [],
        nodes: [],
    ...
    test('should treat companion-only schema as neutral partial instead of mismatch failure', async () => {
      const manifest = {
        metadata: { h1_count: 1 },
        jsonld: [
    ...
    test('should evaluate canonical clarity and crawler accessibility from page directives', async () => {
      const manifest = {
        metadata: { h1_count: 1 },
        jsonld: [],
    ...
    test('should fail crawler accessibility when snippet extraction is blocked', async () => {
      const manifest = {
        metadata: { h1_count: 1 },
        jsonld: [],
    ...
    test('should keep freshness neutral for evergreen content and partial for undated freshness-sensitive content', async () => {
    ...
    test('should not emit heading_topic_fulfillment in deterministic output', async () => {
      const manifest = {
    ...
    test('should anchor missing-alt issue directly to preserved image blocks in post-body content', async () => {
      const manifest = {
        metadata: { h1_count: 1 },
        jsonld: [],
        nodes: [
    ...
    test('should compute intro preflight metrics when enabled', async () => {
      ...
      expect(Array.isArray(manifest.preflight_intro.factual_spans)).toBe(true);
    ...
    test('should stop intro extraction at the first in-body H2 boundary', async () => {
      const manifest = {
        metadata: { h1_count: 1 },
        jsonld: [],
    ...
    test('should stop intro extraction at the first in-body H3 boundary and ignore deeper factual spans', async () => {
      const manifest = {
        metadata: { h1_count: 1 },
        jsonld: [],
        title: 'What Is AEO and GEO?',
    ...
    test('should keep intro checks active when content starts with an immediate H2', async () => {
      const manifest = {
        metadata: { h1_count: 1 },
        jsonld: [],
        title: 'What Is AEO and GEO?',
    ...
    test('should use the recalibrated intro wordcount thresholds', async () => {
    ...
    test('should only count intro factual support when it is local to the factual paragraph', async () => {
      const manifest = {
    ...
    test('should keep intro preflight support data stable when schema suggestion changes but core intro signals do not', async () => {
      const intro = [
    ...
    test('should not trigger FAQ detection for two compact question sections without FAQ intent signals', async () => {
      const manifest = {
        metadata: { h1_count: 1 },
        jsonld: [],
    ...
    test('should keep a single question-led explainer out of FAQ pairs while recording structural hints', async () => {
      const manifest = {
        metadata: { h1_count: 1 },
        jsonld: [],
    ...
    test('should use pseudo FAQ labels to trigger FAQ structural grouping', async () => {
      const manifest = {
        metadata: { h1_count: 1 },
        jsonld: [],
        block_map: [
    ...
    test('should trigger FAQ detection for compact question sections under an explicit FAQ heading', async () => {
      const manifest = {
        metadata: { h1_count: 1 },
        jsonld: [],
        block_map: [
    ...
    test('should still trigger FAQ detection for three compact question sections without explicit FAQ labeling', async () => {
      const manifest = {
        metadata: { h1_count: 1 },
        jsonld: [],
    ...
    test('should not trigger HowTo schema checks for unordered tips in a non-procedural explainer', async () => {
      const manifest = {
        metadata: { h1_count: 1 },
        jsonld: [],
        block_map: [
    ...
    test('should emit per-heading fragmentation messages for over-split H2 sections that hand off immediately', async () => {
      const blockMap = [];
      for (let i = 1; i <= 7; i += 1) {
    ...
    test('should not flag fragmentation when H2 sections introduce nested H3 subsections with framing content', async () => {
      const blockMap = [];
      for (let i = 1; i <= 7; i += 1) {
        blockMap.push({
          node_ref: `h2-${i}`,
    ...
    test('should not flag fragmentation when H2 sections are supported by preserved list blocks', async () => {
      const blockMap = [];
      for (let i = 1; i <= 9; i += 1) {
        blockMap.push({
          node_ref: `h2-${i}`,
    ...
    test('should not flag fragmentation for brief but framed H2 sections', async () => {
      const blockMap = [];
      for (let i = 1; i <= 8; i += 1) {
        blockMap.push({
          node_ref: `h2-${i}`,
