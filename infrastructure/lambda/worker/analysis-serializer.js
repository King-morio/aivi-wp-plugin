...
const SEMANTIC_WHY_IT_MATTERS_VARIANTS = [
    ({ checkName, definitionDescription }) => definitionDescription
        ? `${checkName} checks ${definitionDescription}. Weak coverage here can lower extraction precision and citation confidence.`
        : `${checkName} is a quality signal for answer extraction; weak results can reduce citation confidence.`,
    ({ checkName, definitionDescription }) => definitionDescription
        ? `Answer engines rely on ${checkName} because it reflects ${definitionDescription}. Failing this signal can reduce retrieval reliability.`
        : `Answer engines use ${checkName} as a trust cue; weak performance can lower retrieval reliability.`,
    ({ checkName, definitionDescription }) => definitionDescription
        ? `This finding affects how models interpret ${definitionDescription}. Improving ${checkName} usually improves grounding quality.`
        : `This finding affects grounding quality; stronger ${checkName} signals usually improve answer reliability.`,
    ({ checkName, definitionDescription }) => definitionDescription
        ...
        : `${checkName} influences whether evidence is treated as reliable in machine summaries. Weak signals can limit citability.`
];
...
const normalizeExplanationPack = (rawPack) => {
    ...
    const whatFailed = clampGuidanceText(rawPack.what_failed || rawPack.message || '', 300);
    const whyItMatters = clampGuidanceText(rawPack.why_it_matters || '', 320);
...
const buildSemanticWhyItMatters = ({ checkName, definitionDescription, checkId, runId, instanceIndex }) => {
    ...
    const idx = stableHash(seed) % SEMANTIC_WHY_IT_MATTERS_VARIANTS.length;
    const selected = SEMANTIC_WHY_IT_MATTERS_VARIANTS[idx];
    const raw = typeof selected === 'function'
        ? selected({ checkName, definitionDescription })
        : '';
    return clampGuidanceText(raw, 320);
};
...
const buildDeterministicExplanationPack = ({ checkId, runId, instanceIndex, message, snippet, rewriteTarget, failureReason }) => {
    ...
    const fallbackSteps = [];
    if (snippet) {
        fallbackSteps.push('Edit the quoted passage first, then tighten the surrounding sentence for clarity.');
    } else {
...
const buildIssueExplanationPack = ({
    ...
    const definitionDescription = clampGuidanceText(definitionMeta?.description || '', 240);
    ...
    if (aiPack) {
        ...
        if ((!Array.isArray(merged.how_to_fix_steps) || merged.how_to_fix_steps.length === 0)
            && actionSuggestion
            && !isGenericRepairInstruction(actionSuggestion)
            && !isUiInstructionStep(actionSuggestion)) {
            merged.how_to_fix_steps = [normalizeStepText(actionSuggestion, 220)];
        }
    ...
    const steps = [];
    if (snippet) {
        steps.push('Revise the quoted passage first and keep the change focused on one clear claim sentence.');
    } else {
        steps.push(`Locate the section tied to ${checkName} and revise the weakest passage first.`);
    }
    if (actionSuggestion && !isGenericRepairInstruction(actionSuggestion) && !isUiInstructionStep(actionSuggestion)) {
        steps.push(actionSuggestion);
    } else {
        steps.push('Rewrite for clarity and specificity: remove filler and add one concrete support detail.');
    }
...
const buildRecommendationDepthSteps = ({
    ...
    const seedSteps = Array.isArray(existingSteps)
        ? existingSteps.filter((step) => !isGenericRepairInstruction(step) && !isUiInstructionStep(step))
        : [];
    const steps = [...seedSteps];
    const operation = String(rewriteTarget?.operation || '').toLowerCase().trim();
    const hasActionSuggestion = actionSuggestion && !isGenericRepairInstruction(actionSuggestion) && !isUiInstructionStep(actionSuggestion);
    ...
    if (syntheticFallback) {
        ...
        if (hasActionSuggestion) {
            syntheticSteps.push(actionSuggestion);
        } else if (snippet) {
            syntheticSteps.push('Start from the quoted passage and strengthen one concrete claim with explicit support.');
        } else {
    ...
    } else if (hasActionSuggestion && steps.length < 4) {
        steps.push(actionSuggestion);
    }
...
const enrichRecommendationExplanationPack = (pack, context = {}) => {
    ...
    const whyItMatters = clampGuidanceText(normalized.why_it_matters || context.whyFallback || '', 320);
    ...
    Object.entries(checks).forEach(([checkId, check]) => {
        ...
        if (highlightIntentPolicy.intent !== 'inline') {
            ...
            unhighlightableIssues.push(buildUnhighlightableIssue({
                ...
                sourcePack: policyHighlight?.explanation_pack || check.ai_explanation_pack || check.explanation_pack || null
            }));
            return;
        }
        ...
        highlights.forEach((h, idx) => {
            ...
            if (!nodeRef) {
                unhighlightableIssues.push(buildUnhighlightableIssue({
                    ...
                    nodeRef: '',
                    signature: signature || '',
                    sourcePack: h.explanation_pack || check.ai_explanation_pack || check.explanation_pack || null
                }));
                hasRenderedInstance = true;
                return;
            }
